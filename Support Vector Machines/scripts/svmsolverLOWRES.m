...
if (~exist('../Data/res_16_20/LibData/','dir'))
    mkdir('../Data/res_16_20/LibData/');
end
for fold=1:3
    trainDataFilePath = sprintf('../Data/res_16_20/trPCA_0%i.txt',fold);
    delimeter=' ';
    ...
    classLabelFilePath = sprintf('../Data/res_16_20/TtrPCA_0%i.txt',fold);
    classlabels = dlmread(classLabelFilePath,delimeter);
    % write the data to a directory  'LibData'
    trainData = formatLibSVM(rawtrainCoeffs,'res_16_20',fold,classlabels,'train');
end

% Reading in the raw test data and writing back in LIBSVM Format.
for fold=1:3
    trainDataFilePath = sprintf('../Data/res_16_20/tsPCA_0%i.txt',fold);
    delimeter=' ';
    rawtestCoeffs = dlmread(trainDataFilePath,delimeter);
    trainDataFilePath = sprintf('../Data/res_16_20/valPCA_0%i.txt',fold);
    rawtestCoeffs = [rawtestCoeffs dlmread(trainDataFilePath,delimeter)];
    % only use the coefficients corresponding top 30 eigen vectors
    rawtestCoeffs = rawtestCoeffs(1:30,:);
    % class label filepath
    classLabelFilePath = sprintf('../Data/res_16_20/TtsPCA_0%i.txt',fold);
    classlabels = dlmread(classLabelFilePath,delimeter);
    
    classLabelFilePath = sprintf('../Data/res_16_20/TvalPCA_0%i.txt',fold);
    classlabels = [classlabels dlmread(classLabelFilePath,delimeter)];
    % write the data to a directory  'LibData'
    trainData = formatLibSVM(rawtestCoeffs,'res_16_20',fold,classlabels,'test');
end
